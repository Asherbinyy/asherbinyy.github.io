...
| 0e.21 | **The intro's geometry is still primitives.** Photographed stone now covers it, which is most of the distance between "cheap" and "carved", but the Anubis sentries are boxes and read as boxes close up. A sculpted `.glb` is the fix. The Smithsonian's CC0 scans are the obvious source and their site blocks automated download, so this needs the owner to fetch one or a different source to be found. |
