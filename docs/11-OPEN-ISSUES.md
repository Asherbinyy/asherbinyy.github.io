...
| 0e.23 | **Milestone 7.1 is in: the site reads content the owner publishes.** `ContentRepository` takes an optional `RemoteReader`, prefers it, times it out at two seconds and falls back to the shipped document on every failure. Nothing else is built yet -- the panel, the auth and the media path are 7.2 to 7.5 -- so nothing can publish anything into it and the site behaves exactly as before. That is the point: the read half ships and is proven before the write half exists. |
| 0e.24 | **Milestone 7 is built and needs one deploy from the owner.** The panel is at `/admin`, media upload validates on bytes rather than on the declared type, and content publishes and withdraws behind a token separate from the console one. None of it is live: `CLOUDFLARE_API_TOKEN` is not available in an agent session, so the three commands in `worker/README.md` have to be run by hand -- create the `CONTENT` namespace, paste its id into `wrangler.toml`, set `ADMIN_TOKEN`, deploy. Until then the site behaves exactly as it does today. **Still open:** 7.5's ledger test against remote content, which needs a decision about whether `14-PROVENANCE.md` is regenerated from the change log. |
| 0e.6 | **The ledger grid aligns now.** It was a `Wrap`, which aligns a run to its tallest item and leaves every shorter card sitting in a hole; the roles run from 35 to 184 characters, so a row of three could differ by six lines and the store links -- the only thing on a card anyone clicks -- landed at a different height on every one. Rows share a height and each card settles its links against the bottom, so nothing is truncated and the actionable row is level. **Still open:** the page is an exhibition of procedural marks because there is no real imagery yet. That is 0e.18, and the panel in milestone 7 is what unblocks it. |
...
| 0e.9 | **The map is drawn on a sheet now.** `12-MOTIF-LIBRARY.md` §1 item 9 specified a papyrus ground for `/signal` and it had never been built, so the atlas rendered as pale wireframe on the page background and read as a technical diagram. `papyrus_painter.dart` lays a sheet with a woven fibre tooth and a torn deckled edge behind it, and the coast, graticule and arcs step up a rung to read as ink on it. **The sheet is texture and edge, not a colour:** `01-DESIGN-SYSTEM.md` §2 forbids a fifth hue, so it takes `surfaceRaised`, which is already the palette's raised surface. That turned out better than a compromise -- the light palette's raised surface is a warm cream, so the same code draws stone by lamplight in Kemet and actual papyrus in Deshret. **Still open:** the panel column is empty until a stop is selected. That is deliberate -- the arcs draw in and the viewer chooses -- and three tests plus the step controls depend on it, so changing it is the owner's call rather than a defect to fix. |
...
| ~~0e.12~~ | **Closed. The undergraduate entry says where the programming started.** Third nationally in an inter-university robotics competition in 2015, on Arduino Uno. Only what the owner could confirm: no team size, no project name, no placing beyond third. He has photographs, which are not in the repository. |
| ~~0e.13~~ | **Closed. Russia is on the card.** Tripster now declares `RU`: the employer was Ar++ in Armenia, the product and its client team are Russian, and the card names where the work landed rather than where the desk was. The Ar++ summary also stops implying Tripster was the only thing built there. |
| ~~0e.14~~ | **Closed. The gap after MiNextStep is filled.** MiNextStep ended 2025-08, which the content already had right; freelancing runs from there with no end date, and the schema treats a missing end as ongoing rather than as missing data. |
...
| ~~0e.16~~ | **Closed. The name plays in his own voice.** Beside the gold mark in the hero, as a button rather than anything automatic. Wrapped rather than a row, because at 360px the mark and the control are 18px wider than the column. The label is written on the container with the children silenced: the caption repeats it on screen, so a merged tree announced it twice. |
| 0e.17 | **The climb's controls are rebuilt.** Keyboard: arrows or WASD to steer, space or W to leap, S to dive. Touch: a pad under the frame rather than dragging across the playfield, which put the hand over the shaft it was steering through and left nowhere for a second action. Leap and dive are new mechanics on top of the automatic bounce rather than instead of it, so the extra keys give a player something to do without turning a one-axis game into a two-axis one. **The shaft is a shaft now.** The painter used the whole surface as the playfield, so on a 1440 desktop the climber had the full width to drift through, the ledges were hairline rectangles floating in a void, and the two "walls" were faint lines with nothing behind them. Ice Tower is a narrow tower. `AscentPainter.shaftOf` returns a centred column, the rock either side is drawn as masonry that scrolls past, the camera holds 20 metres instead of 26, and ledges are cut blocks with a lit top edge rather than rules. The frame is still filled edge to edge, which was the owner's actual complaint, but it is filled with the temple rather than with more playfield. Piers grow from zero above 460px so no phone ever sees one. **Still open:** density and pace as *gameplay* -- ledge gap and width are unchanged, because tuning them changes difficulty and that is a decision to be felt rather than guessed at. |
| ~~0e.22~~ | **Closed. The site was scattering random letters.** The owner asked to "fix the hieroglyphic language, letters". The defect: four painters -- the background field, the station cards, the wall and the game shaft -- picked at random from the five Egyptian uniliteral signs and drew the result. Those are phonetic letters, and a random arrangement of them spells nonsense to anyone who reads them. `12-MOTIF-LIBRARY.md` §0 rule 3 had forbidden exactly this, and §4 contained a written defence of it: that a grid does not read as a sentence the way a line does. That argument was wrong and is withdrawn. Worse, the one legitimate use of the alphabet -- the name cartouche -- had already been removed on the owner's instruction, so the only Egyptian writing left on the site was the forbidden kind. Replaced by `ornament_paths.dart`: kheker cresting, block border, coil, rosette and lotus, all decoration in the record and none of it carrying a sound value. `glyph_paths.dart` and `cartouche_painter.dart` are deleted rather than left unreferenced. |
| ~~0e.21~~ | **Closed. The sentries are a real statue now.** They were nine boxes each and read as nine boxes. They are a photogrammetry scan of a statue of Ra-Horakhty from Wikimedia Commons, CC BY-SA 4.0, decimated from 102,714 triangles to 21,092 by `tool/mesh/decimate_stl.py` and carried in a 184KB purpose-built container rather than through a 100KB glTF loader. The scan arrived leaning seven degrees, which the tool now corrects by fitting the model's own axis; and it is untextured on purpose, because photogrammetry has no UVs and the carving is its detail. Attribution is on screen and in `14-PROVENANCE.md` §3e. The Smithsonian, Poly Haven and Three D Scans were all tried first and all failed, for reasons recorded there. |
